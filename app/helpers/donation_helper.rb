module DonationHelper
    ...
    def tweet_button
      tweet_text = "I'm #{Settings.primary_stat_verb} number #{number_with_delimiter Gift.supporters, :delimiter => ","} #{Settings.tweet_text}!"
    ...
    def encoded_root_url
      raw URI.encode "#{request.scheme}://#{request.host}/donation"
    end
  
    def sold_out(donation_amount_option)
      donation_amount_option.limit > -1 and donation_count(donation_amount_option) >= donation_amount_option.limit
    end
  
    def donation_count(contribution_option)
      Donation.where(donation_amount_option_id: donation_amount_option).count(:token) # count of orders that have a token from amazon and are for this contribution option
    end
  end